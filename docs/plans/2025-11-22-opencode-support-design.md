...
Add full superfast support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port superfast to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/superfast-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/superfast.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/superfast/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/superfast-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "superfast:brainstorming")')
  }),
...
1. **Inject using-superfast content**
   - Full content of the using-superfast skill
...
```javascript
// .opencode/plugin/superfast.js
const skillsCore = require('../../lib/skills-core');
const path = require('path');
const fs = require('fs');
const { z } = require('zod');

export const SuperfastPlugin = async ({ client, directory, $ }) => {
  const superfastDir = path.join(process.env.HOME, '.config/opencode/superfast');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingSuperfast = await readSkill('using-superfast');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingSuperfast}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── superfast-codex        # UPDATED: Use skills-core
│   ├── superfast-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── superfast.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/superfast-codex`
...
2. Update `.codex/superfast-codex` to use shared core
...
1. Create `.opencode/plugin/superfast.js`
