...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superfast:executing-plans to implement this plan task-by-task.

**Goal:** Add full superfast support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/superfast-codex` (lines 40-74)
...
- Reference: `.codex/superfast-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'superfast' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/superfast-codex` (lines 212-280)
 ...
 * (personal skills override superfast skills).
 *
 * @param {string} skillName - Name like "superfast:brainstorming" or "my-skill"
 * @param {string} superfastDir - Path to superfast skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, superfastDir, personalDir) {
    // Strip superfast: prefix if present
    const forceSuperfast = skillName.startsWith('superfast:');
    const actualSkillName = forceSuperfast ? skillName.replace(/^superfast:/, '') : skillName;

    // Try personal skills first (unless explicitly superfast:)
    if (!forceSuperfast && personalDir) {
    ...
    // Try superfast skills
    if (superfastDir) {
        const superfastPath = path.join(superfastDir, actualSkillName);
        const superfastSkillFile = path.join(superfastPath, 'SKILL.md');
        if (fs.existsSync(superfastSkillFile)) {
            return {
                skillFile: superfastSkillFile,
                sourceType: 'superfast',
                skillPath: actualSkillName
            };
        }
    }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/superfast-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/superfast-codex` (add import at top)
...
Run: `node -c .codex/superfast-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/superfast-codex
...
**Files:**
- Modify: `.codex/superfast-codex` (lines 40-74)
...
Run: `.codex/superfast-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/superfast-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/superfast-codex` (lines 16-38, approximately)
...
Run: `.codex/superfast-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/superfast.js`
...
/**
 * Superfast plugin for OpenCode.ai
 *
...
const superfastSkillsDir = path.join(homeDir, '.config/opencode/superfast/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const SuperfastPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/superfast.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/superfast.js
...
**Files:**
- Modify: `.opencode/plugin/superfast.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "superfast:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > superfast)
...
Run: `node -c .opencode/plugin/superfast.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the superfast and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const superfastSkills = skillsCore.findSkillsInDir(
            superfastSkillsDir,
            'superfast',
            3
          );
          ...
          const allSkills = [...personalSkills, ...superfastSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install superfast skills to ~/.config/opencode/superfast/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'superfast:';
...
```javascript
    'session.started': async () => {
      // Read using-superfast skill content
      const usingSuperfastPath = skillsCore.resolveSkillPath(
        'using-superfast',
        superfastSkillsDir,
        personalSkillsDir
      );

      let usingSuperfastContent = '';
      if (usingSuperfastPath) {
        const fullContent = fs.readFileSync(usingSuperfastPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingSuperfastContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Superfast skills: \`superfast:skill-name\` (from ~/.config/opencode/superfast/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/superfast')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/superfast && git pull` to update superfast.' :
        '';
...
**Below is the full content of your 'superfast:using-superfast' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingSuperfastContent}
...
```markdown
# Installing Superfast for OpenCode
...
### 1. Install Superfast Skills

```bash
# Clone superfast skills to OpenCode config directory
mkdir -p ~/.config/opencode/superfast
git clone https://github.com/FastBuilderAI/superfast.git ~/.config/opencode/superfast
```

### 2. Install the Plugin

The plugin is included in the superfast repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/superfast/.opencode/plugin/superfast.js`
...
ln -s ~/.config/opencode/superfast/.opencode/plugin/superfast.js .opencode/plugin/superfast.js
```
...
```
use use_skill tool with skill_name: "superfast:brainstorming"
```
...
Personal skills override superfast skills with the same name.

## Updating

```bash
cd ~/.config/opencode/superfast
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/superfast/.opencode/plugin/superfast.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/superfast/skills`
...
- Documentation: https://github.com/FastBuilderAI/superfast
```
...
Superfast works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/superfast-codex use-skill superfast:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/superfast-codex bootstrap | head -30`
...
ls -l .opencode/plugin/superfast.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/superfast.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/superfast-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/superfast-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/superfast.js` created with tools and hooks
