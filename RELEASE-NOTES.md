...
- **Added `package.json`** so OpenCode can install superfast as an npm package from git.
...
- `GEMINI.md` @imports `using-superfast` skill and tool mapping table at session start
- Gemini CLI tool mapping reference (`skills/using-superfast/references/gemini-tools.md`) — translates Claude Code tool names (Read, Write, Edit, Bash, etc.) to Gemini CLI equivalents (read_file, write_file, replace, etc.)
...
- Specs (brainstorming output) now save to `docs/superfast/specs/YYYY-MM-DD-<topic>-design.md`
- Plans (writing-plans output) now save to `docs/superfast/plans/YYYY-MM-DD-<feature-name>.md`
...
Writing-plans no longer offers a choice between subagent-driven and executing-plans. On harnesses with subagent support (Claude Code, Codex), subagent-driven-development is required. Executing-plans is reserved for harnesses without subagent capability, and now tells the user that Superfast works better on a subagent-capable platform.
...
- `lib/brainstorm-server/` — WebSocket server with browser helper library, session management scripts, and dark/light themed frame template ("Superfast Brainstorming" with GitHub link)
...
- Design spec and implementation plan in `docs/superfast/`
...
Added explicit priority ordering to using-superfast:

1. User's explicit instructions (CLAUDE.md, AGENTS.md, direct requests) — highest priority
2. Superfast skills — override default system behavior
...
Added `<SUBAGENT-STOP>` block to using-superfast. Subagents dispatched for specific tasks now skip the skill instead of activating the 1% rule and invoking full skill workflows.
...
This fix should dramatically improve superfast skills compliance and should reduce the chances of Claude entering its native plan mode unintentionally.
...
**Using-superfast workflow graph intercepts EnterPlanMode**
...
Changed `async: true` to `async: false` in hooks.json. When async, the hook could fail to complete before the model's first turn, meaning using-superfast instructions weren't in context for the first message.
...
The `superfast-codex` bootstrap CLI, Windows `.cmd` wrapper, and related bootstrap content file have been removed. Codex now uses native skill discovery via `~/.agents/skills/superfast/` symlink, so the old `use_skill`/`find_skills` CLI tools are no longer needed.
...
The synchronous SessionStart hook blocked the TUI from entering raw mode on Windows, freezing all keyboard input. Running the hook async prevents the freeze while still injecting superfast context.
...
- Windows doesn't respect shebangs, so directly invoking the extensionless `superfast-codex` script triggered an "Open with" dialog. All invocations now prefixed with `node`.
...
Superfast for OpenCode now uses OpenCode's native `skill` tool instead of custom `use_skill`/`find_skills` tools. This is a cleaner integration that works with OpenCode's built-in skill discovery.

**Migration required:** Skills must be symlinked to `~/.config/opencode/skills/superfast/` (see updated installation docs).
...
**Strengthened using-superfast skill for explicit skill requests**
...
The underlying skills (`superfast:brainstorming`, `superfast:executing-plans`, `superfast:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
Fixed a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-superfast` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.

- Added "How to Access Skills" section to `using-superfast`
- Changed "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `superfast:brainstorming`)
...
**Skill priority in using-superfast**
...
- **Rationalizations table** in using-superfast - Scannable format including new entries: "I need more context first", "Let me explore first", "This feels productive"
...
- **OpenCode Bootstrap Refactor**: Switched from `chat.message` hook to `session.created` event for bootstrap injection
  ...
  - Explicitly tells the model that using-superfast is already loaded to prevent redundant skill loading
...
- **OpenCode Support**: Native JavaScript plugin for OpenCode.ai
  ...
  - Three-tier skill priority: project > personal > superfast
...
- Optimized superfast bootstrap to eliminate redundant skill execution. The `using-superfast` skill content is now provided directly in session context, with clear guidance to use the Skill tool only for other skills. This reduces overhead and prevents the confusing loop where agents would execute `using-superfast` manually despite already having the content from session start.
...
- Added unified `superfast-codex` script with bootstrap/use-skill/find-skills commands
- Cross-platform Node.js implementation (works on Windows, macOS, Linux)
- Namespaced skills: `superfast:skill-name` for superfast skills, `skill-name` for personal
- Personal skills override superfast skills when names match
...
- Updated terminology: "Superfast skills" instead of "Core skills"

### Files Added
- `.codex/INSTALL.md` - Installation guide for Codex users
- `.codex/superfast-bootstrap.md` - Bootstrap instructions with Codex adaptations
- `.codex/superfast-codex` - Unified Node.js executable with all functionality

**Note:** Codex support is experimental. The integration provides core superfast functionality but may require refinement based on user feedback.

## v3.2.3 (2025-10-23)

### Improvements

**Updated using-superfast skill to use Skill tool instead of Read tool**
...
### Files Changed
- Updated: `skills/using-superfast/SKILL.md` - Changed tool references from Read to Skill

## v3.2.2 (2025-10-21)

### Improvements

**Strengthened using-superfast skill against agent rationalization**
...
### Files Changed
- Updated: `skills/using-superfast/SKILL.md` - Added three layers of enforcement to prevent skill-skipping rationalization
...
- Added `superfast:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `superfast:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code-reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting-code-review/SKILL.md` - References to `superfast:code-reviewer`
- Updated: `skills/subagent-driven-development/SKILL.md` - References to `superfast:code-reviewer`
...
- All internal skill references now use `superfast:` namespace prefix
- Updated format: `superfast:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/superfast:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
# Superfast v2.0.0 Release Notes

## Overview

Superfast v2.0 makes skills more accessible, maintainable, and community-driven through a major architectural shift.

The headline change is **skills repository separation**: all skills, scripts, and documentation have moved from the plugin into a dedicated repository ([obra/superfast-skills](https://github.com/obra/superfast-skills)). This transforms superfast from a monolithic plugin into a lightweight shim that manages a local clone of the skills repository. Skills auto-update on session start. Users fork and contribute improvements via standard git workflows. The skills library versions independently from the plugin.
...
**The biggest change:** Skills no longer live in the plugin. They've been moved to a separate repository at [obra/superfast-skills](https://github.com/obra/superfast-skills).

**What this means for you:**

- **First install:** Plugin automatically clones skills to `~/.config/superfast/skills/`
...
1. Your old `~/.config/superfast/.git` will be backed up to `~/.config/superfast/.git.bak`
2. Old skills will be backed up to `~/.config/superfast/skills.bak`
3. Fresh clone of obra/superfast-skills will be created at `~/.config/superfast/skills/`

### Removed Features

- **Personal superfast overlay system** - Replaced with git branch workflow
- **setup-personal-superfast hook** - Replaced by initialize-skills.sh
...
- Clones obra/superfast-skills on first run
...
- `SUPERPOWERS_SKILLS_ROOT` set to `~/.config/superfast/skills`
...
- Removed obsolete setup-personal-superfast call from session-start
...
- Prominent link to superfast-skills repo
...
**Removed:**
- `skills/` directory (82 files) - Now in obra/superfast-skills
- `scripts/` directory - Now in obra/superfast-skills/skills/using-skills/
- `hooks/setup-personal-superfast.sh` - Obsolete

**Modified:**
- `hooks/session-start.sh` - Use skills from ~/.config/superfast/skills
...
- PR #2: Personal superfast overlay system (later replaced)
...
1. **Backup your personal skills** (if you have any):
   ```bash
   cp -r ~/.config/superfast/skills ~/superfast-skills-backup
   ```
...
- Skills repository is now at https://github.com/obra/superfast-skills
...
**Full Changelog:** https://github.com/obra/superfast/compare/dd013f6...main
**Skills Repository:** https://github.com/obra/superfast-skills
